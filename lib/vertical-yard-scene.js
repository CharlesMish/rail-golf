import {MeshBuilder,PhysicsAggregate,PhysicsShapeType,Color3,Vector3} from '@babylonjs/core';
...
export function buildVerticalYard(scene,root,materials,shadows,options={}){
  ...
  for(const record of options.solids??VERTICAL_SOLIDS){
    ...
    if(record.zone==='ladder'){
      // A narrow rear stanchion makes each timber cheek industrial structure,
      // not a floating token. Its visible mass is also an ordinary static box.
      const anchor=Vector3.TransformCoordinates(new Vector3(0,0,record.d/2+.6),mesh.computeWorldMatrix(true));
      const h=record.y+record.h/2;
      const post=physical(box(record.id+'-stanchion',{x:anchor.x,y:h/2,z:anchor.z,w:.65,h,d:.65},'steel'));
      post.metadata={lineFeature:{id:record.id+'-stanchion',kind:'range-timber',label:'RETURN STANCHION REBOUND'},rangeZone:'ladder'};
      features.set(record.id+'-stanchion',post);
      // Inset, pale timber on both usable broad sides; extents match the box.
      for(const side of [-1,1]){
        const face=box(record.id+'-face',{x:0,y:0,z:side*(record.d/2+.018),w:record.w-.6,h:record.h-.6,d:.025},'timber');face.parent=mesh;face.material=receiverTimber;
      }
    }
