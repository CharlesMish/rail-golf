...
export function buildDiverterLab(scene,root,materials,shadows,initial=DEFAULT_FLOOR,options={}) {
  const FLOOR=options.floor ?? DEFAULT_FLOOR_GEOMETRY, SWITCH=options.switch ?? DEFAULT_SWITCH_GEOMETRY, TARGET=options.target === null ? null : (options.target ?? DEFAULT_TARGET);
  const states=options.states ?? FLOOR_STATES;
  const bodies=new Set();
  const box=(name,w,h,d,x,y,z,material,parent=root)=>{
    const mesh=MeshBuilder.CreateBox((options.prefix ?? 'diverter-')+name,{width:w,height:h,depth:d},scene);
  ...
  if(options.switchLabel)switchMesh.metadata={lineSwitchLabel:options.switchLabel};
  ...
  const setState=next=>{
    ...
    floor.metadata={lineFeature:options.feature ?? {id:'loading-dock',kind:'dock',label:'LOADING DOCK REBOUND'}};
    ...
    if(options.palletBoards)for(let i=1;i<8;i++)box('board-joint-'+i,FLOOR.width-.15,.012,.035,0,FLOOR.height/2+.004,-FLOOR.depth/2+i*FLOOR.depth/8,'machine',floor);
    for(const x of options.palletBoards ? [-.36,.36].map(t=>t*FLOOR.width) : options.overlay ? [-.4,-.2,0,.2,.4].map(t=>t*FLOOR.width) : [-7,-3.5,0,3.5,7]) box('floor-stripe-'+x,.30,.045,FLOOR.depth-.3,x,FLOOR.height/2+.03,0,states[state].material,floor);
