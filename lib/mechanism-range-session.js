...
export function createMechanismSession(scene,world,emit=()=>{}){
  ...
  return {
    ...
    fire(setup){
      ...
      if(!Number.isInteger(setup.railIndex)||setup.railIndex<0||setup.railIndex>2||!Number.isFinite(setup.yaw)||Math.abs(setup.yaw)>70||!Number.isFinite(setup.elevation)||setup.elevation<5||setup.elevation>85||!Number.isFinite(setup.charge)||setup.charge<0||setup.charge>1)return false;
