...
export function createMechanismSession(scene,world,emit=()=>{},options={}){
  ...
  const finish=reason=>{
    ...
    last={setup:{...flight.setup},environment:options.environment?structuredClone(flight.start):{floor:flight.start},endEnvironment:options.environment?structuredClone(options.environment.snapshot()):{floor:world.mechanism.state},reason,elapsed:flight.elapsed,evidence:flight.evidence.map(e=>({...e})),contacts:flight.contacts.map(e=>({...e})),position:flight.mesh.position.asArray()};
    options.onResolve?.(flight,reason);
  ...
  return {
    ...
    fire(setup){
      ...
      options.prepareLaunch?.(setup);
      clear();const station=options.station?.(setup)??RANGE_STATION,muzzle=stationMuzzle(setup,station),aim=stationAim(setup,station);muzzle.y+=station.y??0;
      ...
      flight={id:++counter,mesh,aggregate,setup:{...setup},start:options.environment?structuredClone(options.environment.snapshot()):world.mechanism.state,elapsed:0,slowSince:null,ended:null,ground:false,tracker:createRedirectTracker(),evidence:[],contacts:[]};
      ...
      aggregate.body.getCollisionObservable().add(event=>{
        ...
        const contact=options.contact?options.contact(other,event.point,flight):world.mechanism.contact(other,event.point,flight.id);
      ...
      options.onLaunch?.(flight);emit({kind:'launch',state:flight.start});return true;
    },
    beforeStep(){if(flight&&!flight.ended){options.beforeStep?.(flight,1/120);flight.tracker.beginStep(flight.aggregate.body.getLinearVelocity(),flight.elapsed);}else options.beforeIdleStep?.(1/120);},
    afterStep(){
      if(!flight||flight.ended)return;
      flight.elapsed+=1/120;
      options.afterStep?.(flight,1/120);const state=options.flush?options.flush():world.mechanism.flush();if(state)emit({kind:'state',state});
      ...
      const end=(options.end??rangeEnd)(p,v,flight.elapsed,flight.slowSince);if(end)finish(end);
    },
    action(action){
      if(action==='recall'&&!last)return null;
      finish('interrupted');const setup=last?.setup;
      if(options.environment){if(action==='reset')options.environment.reset();else if(action==='recall')options.environment.restore(last.environment);}else world.mechanism.setState(floorForAction(world.mechanism.state,action,last?.environment));
      clear();options.onAction?.(action);emit({kind:'state',state:options.environment?options.environment.snapshot():world.mechanism.state});return setup?{...setup}:null;
    },
    dispose(){clear();},
  };
}
