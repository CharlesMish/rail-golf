...
export const PLACEHOLDER_RULES = Object.freeze([
 ...
 {id:'seat',tier:'FINISH',label:'TARGET SEAT',points:1000,sequence:['target-seat']},
...
export function scoreLine(ledger,rules=PLACEHOLDER_RULES){
 const tokens=ledger.map((event,index)=>({tokens:tokensFor(event),index}));
 ...
 if(seat&&!ledger.slice(0,seat.index).some(e=>e.kind!=='ruling'&&!(e.kind==='contact'&&e.terminal===true)))tokens[seat.index].tokens.push('direct');
 const awards=[];
 ...
 return {ruleSet:'placeholder-v2',nonCanonical:true,awards,total:awards.reduce((n,a)=>n+a.points,0),
  ignored:ledger.flatMap((event,index)=>used.has(index)?[]:[{index,reason:tokensFor(event).length?'Repeated or incomplete family':'Raw evidence — no qualified claim'}])};
}

export function normalizeLineEvidence(value){
 if(!Array.isArray(value)||value.length>4096)return [];
 const kinds=['contact','pad-activation','switch-use','ruling','token','redirect'];
 if(!value.every(e=>e&&kinds.includes(e.kind)&&['surface','body','feature','label'].every(k=>e[k]===undefined||(typeof e[k]==='string'&&e[k].length<=180))&&
  ...
  (['turn','freeSeconds','separation'].every(k=>e[k]===undefined||Number.isFinite(e[k])))&&
 ...
 return value.map(e=>({kind:e.kind,...Object.fromEntries(['feature','label','incoming','outgoing','turn','freeSeconds','separation','terminal'].filter(k=>e[k]!==undefined).map(k=>[k,e[k]])),...(e.surface!==undefined?{surface:e.surface}:{}),...(e.body!==undefined?{body:e.body}:{}),
