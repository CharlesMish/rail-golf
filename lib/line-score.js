...
export function scoreLine(ledger,rules=PLACEHOLDER_RULES,variety=VARIETY_RULE,runRule=RUN_RULE){
 ...
 for(const rule of rules){
  ...
  if(next===rule.sequence.length)awards.push({id:rule.id,tier:rule.tier,label:(rule.id==='seat'||rule.id==='switch')?(ledger[evidence.at(-1)].label??rule.label):rule.label,points:rule.points,evidence});
 }
...
export function recordLineContact(ledger,node,point,diverterContact){
 ...
 if(diverterContact?.kind?.startsWith('switch-'))appendLineEvidence(ledger,{kind:'switch-use',state:diverterContact.kind==='switch-a'?'A':'B',surface:'dock-switch',...(node.metadata?.lineSwitchLabel?{label:node.metadata.lineSwitchLabel}:{})});
}
