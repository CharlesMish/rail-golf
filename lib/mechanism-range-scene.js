...
export function buildMechanismRange(scene,root,materials,shadows){
  ...
  // Exposed drive pipe visually connects actuator and hinge. It is scenery, not a new switch.
  box('drive-pipe',17,1.1,31.5,.24,.24,5,'steel');
  box('drive-pipe-cross',12,1.1,34,10,.24,.24,'steel');
  ...
  return {
    ...
    dispose(){mechanism.dispose();for(const b of bodies)b.dispose();},
