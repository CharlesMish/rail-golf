...
export type DiverterOptions={prefix?:string;palletBoards?:boolean;switchLabel?:string;feature?:{id:string;kind:string;label:string};overlay?:boolean;supports?:boolean;states?:Record<FloorState,{pitch:number;roll?:number;yOffset?:number;material:string}>;floor?:{x:number;y:number;z:number;width:number;height:number;depth:number};switch?:{x:number;y:number;z:number;width:number;height:number;depth:number};target?:{id:string;x:number;z:number;radius:number}|null};
