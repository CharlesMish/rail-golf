...
export const RANGE_MECHANISMS = Object.freeze({
  // Moved downrange from z 66 -> 90: the gate used to sit directly between
  // the address camera and AMBER ROOST (6.5, 79) on all three rails, blocking
  // every view of that target. z 90 clears every target's sightline while
  // staying in the Ruckus Line arc on the way to FAR BELL (4, 120).
...
export const REFERENCE_SHOTS = Object.freeze({
  ...
  // Retuned for the gate move (z 66 -> 90): higher, steeper arc so the ball
  // is still comfortably airborne (not yet on the ground) when it reaches the
  // new gate position, then the breach lift carries it on to FAR BELL.
...
export const HOLES = Object.freeze([
  ...
  Object.freeze({
    ...
    breachRecoveryY: 8,
    perfectAvailable: true,
    defaultShot: Object.freeze({ railIndex: 2, yaw: 1.2, elevation: 44.5 }),
  }),
]);
