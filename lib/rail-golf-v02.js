...
export const RANGE_MECHANISMS = Object.freeze({
  // Past AMBER ROOST so the breach cannot occlude its address sightline.
  breach: Object.freeze({
    x: 7.5, z: 90, halfWidth: 2.4, halfDepth: 1.25, minY: 0.25, maxY: 6.4,
  }),
  bank: Object.freeze({
    // Extend toward the tee: more approach angles reach the wall before landing.
    x: -10.5, z: 38, halfWidth: 0.75, halfDepth: 26, minY: 0.15, maxY: 8.4,
  }),
  boost: Object.freeze({
...
export const REFERENCE_SHOTS = Object.freeze({
  "open-seat": Object.freeze({ railIndex: 1, yaw: 8.6, elevation: 20, charge: legacyChargeToCurrent(0.2) }),
  "timber-bank": Object.freeze({ railIndex: 0, yaw: -15.5, elevation: 20, charge: legacyChargeToCurrent(0.73) }),
  "hot-skip": Object.freeze({ railIndex: 0, yaw: -2, elevation: 64, charge: legacyChargeToCurrent(0.3) }),
  // Verified with grounded, settled Havok crates (including projectile contact).
  "ruckus-line": Object.freeze({ railIndex: 2, yaw: 1.2, elevation: 44.5, charge: legacyChargeToCurrent(0.458) }),
});
...
export const ADDRESS_LAB_MODES = Object.freeze(["gifted", "near", "neutral"]);
export const ADDRESS_LAB_ADDRESSES = Object.freeze({
  gifted: Object.freeze({ railIndex: 2, yaw: -14.7, elevation: 36 }),
  near: Object.freeze({ railIndex: 2, yaw: -10, elevation: 32 }),
  neutral: Object.freeze({ railIndex: 1, yaw: 0, elevation: 28 }),
});

export function parseAddressLabMode(value) {
  return value === "gifted" || value === "near" || value === "neutral" ? value : null;
}

export function resolveAddressLabFromSearch(search) {
  ...
  if (typeof URLSearchParams !== "undefined" && search instanceof URLSearchParams) {
    ...
    if (typeof search.get === "function") {
      params = search;
    } else if (Object.prototype.hasOwnProperty.call(search, ADDRESS_LAB_QUERY_KEY)) {
      return parseAddressLabMode(search[ADDRESS_LAB_QUERY_KEY]);
    } else {
      return null;
    }
  } else {
  ...
  const values = typeof params.getAll === "function"
    ? params.getAll(ADDRESS_LAB_QUERY_KEY)
    : [params.get(ADDRESS_LAB_QUERY_KEY)];
  if (values.length !== 1) return null;
  return parseAddressLabMode(values[0]);
}

export function resolveOpeningAddress(hole, labMode, restoration = {}) {
  if (restoration.restore && restoration.memory) return restoration.memory;
  const mode = parseAddressLabMode(labMode);
  if (mode && hole.id === ADDRESS_LAB_HOLE_ID) return ADDRESS_LAB_ADDRESSES[mode];
  return hole.defaultShot;
}

export function resolveSessionStartHoleIndex(labMode, resumeIndex = 0) {
  const mode = parseAddressLabMode(labMode);
  if (!mode) return resumeIndex;
  const index = HOLES.findIndex((hole) => hole.id === ADDRESS_LAB_HOLE_ID);
  return index < 0 ? resumeIndex : index;
}

export function addressLabChipLabel(labMode) {
  const mode = parseAddressLabMode(labMode);
  return mode ? `ADDRESS LAB · ${mode.toUpperCase()}` : null;
}
...
export const HOLES = Object.freeze([
  ...
  Object.freeze({
    ...
    kicker: "Mechanism Range",
    parLabel: "Bank / Seat",
    instruction: "Bank off the near timber face, then land on Amber Roost.",
    target: RANGE_TARGETS[1],
    requiredTags: Object.freeze(["bank"]),
    perfectAvailable: true,
    defaultShot: Object.freeze({ railIndex: 0, yaw: -15.5, elevation: 20 }),
  }),
  Object.freeze({
    ...sharedRange,
    id: "hot-skip",
...
export function speedToCharge(speed) {
  return clamp((speed - RAIL_RULES.minSpeed) / (RAIL_RULES.maxSpeed - RAIL_RULES.minSpeed), 0, 1);
}

// Preserve the physical launch of pre-v2 saved lines and regression fixtures.
export function legacyChargeToCurrent(charge) {
  return speedToCharge(22 + 21 * clamp(charge, 0, 1));
}

// Sample the input clock at release, independently of the render cadence.
export function chargeFromHold(milliseconds) {
  if (!Number.isFinite(milliseconds)) return 0;
  const seconds = clamp(milliseconds / 1000, 0, RAIL_RULES.chargeSeconds);
  // Continuous control: 0–80% in two seconds, then a slower final second.
  return seconds <= 2 ? seconds * .4 : .8 + (seconds - 2) * .2;
}
...
export function segmentSphereAabbIntersection(
  ...
  // Rotate the sweep into an authored bank's local frame. The returned time
  // remains on the original segment; visible geometry uses the same yaw.
  if (box.yaw) {
    const angle = box.yaw * Math.PI / 180;
    const local = (point) => ({
      x: (point.x - box.x) * Math.cos(angle) - (point.z - box.z) * Math.sin(angle),
      y: point.y,
      z: (point.x - box.x) * Math.sin(angle) + (point.z - box.z) * Math.cos(angle),
    });
    return segmentSphereAabbIntersection(local(start), local(end), { ...box, x: 0, z: 0, yaw: 0 }, radius);
  }
...
// Order evidence on the physics segment. A later volume must never rewrite an
// earlier first kiss. Callers stop processing after a velocity-changing impulse:
// the remaining segment describes the old trajectory, not the relaunched shot.
export function collectShotStepEvents(start, end, hole, tags = []) {
  const events = [];
  const addVolume = (kind, volume) => {
    if (!volume) return;
    const amount = segmentSphereAabbIntersection(start, end, volume);
    if (amount !== null) events.push({ kind, amount, point: pointOnSegment(start, end, amount) });
  };
  if (hole.banks) {
    for (const bank of hole.banks) if (!tags.includes(bank.id)) addVolume(bank.id, bank);
    if (!tags.includes("boost") && end.y < start.y) addVolume("boost", hole.boost);
  } else {
    if (!tags.includes("bank")) addVolume("bank", RANGE_MECHANISMS.bank);
    if (!tags.includes("boost") && end.y < start.y) addVolume("boost", RANGE_MECHANISMS.boost);
  }
  if (!tags.includes("breach")) addVolume("breach", hole.breach);
  addVolume("wet", hole.water);
  const landing = landingIntersection(start, end);
  if (landing) {
    events.push({ kind: "first-kiss", amount: (start.y - landing.y) / (start.y - end.y), point: landing });
  }
  const priority = { wet: 0, bank: 1, boost: 1, breach: 1, "first-kiss": 2 };
  return events.sort((a, b) => a.amount - b.amount || (priority[a.kind] ?? 1) - (priority[b.kind] ?? 1));
}
...
export function tagsMeetChallenge(hole, tags = []) {
  if (!hole.orderedTags) return hole.requiredTags.every((tag) => tags.includes(tag));
  let cursor = -1;
  return hole.requiredTags.every((tag) => {
    cursor = tags.indexOf(tag, cursor + 1);
    return cursor >= 0;
  });
}
...
export function formatMiss(hole, point) {
  const dx = point.x - hole.target.x, dz = point.z - hole.target.z;
  const heading = (hole.station?.yaw ?? 0) * Math.PI / 180;
