...
export function addressCameraFrame({ railX, yawDegrees, target, portrait = false }) {
  const yaw = (yawDegrees * Math.PI) / 180;
  const horizontal = { x: Math.sin(yaw), z: Math.cos(yaw) };
  const basePosition = {
    x: railX - horizontal.x * 15,
    y: 7.8,
    z: -horizontal.z * 15,
  };
  const baseTarget = {
    x: railX + horizontal.x * 34,
    y: 3.6,
    z: horizontal.z * 34,
  };
  const portraitShiftX = portrait ? -8.5 - basePosition.x : 0;
  return {
    position: {
      ...basePosition,
      x: basePosition.x + portraitShiftX,
    },
    target: {
      ...baseTarget,
      x: baseTarget.x + portraitShiftX,
    },
  };
}
