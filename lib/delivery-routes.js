...
export const withSharedYardPad = hole => ['mill-delivery','switchback-gallery','lumber-cascade','courtyard-diverter'].includes(hole.id) ? {...hole, boost:COURTYARD_SKIP_PAD} : hole;

export function collectDeliveryStepEvents(start, end, hole, tags = [], routes = []) {
  const events = collectShotStepEvents(start, end, withSharedYardPad(hole), tags);
