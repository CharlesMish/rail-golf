import {acceptsLineStation} from './line-lab.js';
import {normalizeLineEvidence,normalizeLineReceipt} from './line-score.js';
import { isFloorState } from './diverter-lab.js';
...
const CONTACTS = new Set(['bank','bank-a','bank-b','boost','breach','step-a','step-b','step-c','sky','mill','first-kiss','wet','floor-a','floor-b','switch-a','switch-b']);
...
export function lineFamily(line) {
  ...
  const family=kinds.join(' → ') || 'clear carry';
  return line.environment ? 'FLOOR '+line.environment.floor+' · '+family : family;
}
...
export function normalizeShotLibrary(value, holes, options={}) {
  const library = {};
  ...
  for (const hole of holes) {
    ...
    const normalize = line => {
      if (!line || line.holeId !== hole.id || !acceptsLineStation(hole,line.stationId) || line.windId !== hole.wind.id) return null;
      const lab = options.environmentRequired || ['diverter-floor','courtyard-diverter'].includes(hole.id);
      if(lab && (!isFloorState(line.environment?.floor) || !isFloorState(line.environmentAfter?.floor))) return null;
      ...
      return { ...(lab ? {build:typeof line.build==='string'&&/^([a-f0-9]{7,40}(-dirty)?|unknown)$/.test(line.build)?line.build:'unknown',ledger:normalizeLineEvidence(line.ledger),lineReceipt:normalizeLineReceipt(line.lineReceipt),environment:{floor:line.environment.floor},environmentAfter:{floor:line.environmentAfter.floor}} : {}), holeId:hole.id, stationId:line.stationId, windId:line.windId, projectileId:line.projectileId,
