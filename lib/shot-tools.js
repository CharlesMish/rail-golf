...
export function launchCharge(mode, selected, milliseconds, max = false) {
  if(max)return 1;
...
// Input authority only. Retry retains the latch; explicit precision restores override it.
export function maxLatchAfter(enabled,action){
  if(action==='toggle')return !enabled;
  if(['recall','import','set'].includes(action))return false;
  return enabled;
}
