...
export function createActionTrace({storage,build,session=globalThis.crypto.randomUUID(),limits=ACTION_TRACE_LIMITS,context=()=>({})}){
 ...
 return {
  append(entry){entries.push(clone({category:'action',...entry,...context(),build,session,sequence:++sequence,timestamp:new Date().toISOString(),monotonicMs:performance.now()}));
  ...
  summary(){return clone({session,...context(),count:entries.length});},
  export(scope='all'){
   if(!['mount','document','tab','all'].includes(scope))throw Error('Unknown action trace scope');
   let data;try{data=readAll();}catch(error){warning='Action trace read unavailable: '+String(error);data={entries,retired};}
   const active={session,build,...context()};
   const selected=data.entries.filter(e=>scope==='all'||(scope==='mount'?Boolean(active.mountId)&&e.mountId===active.mountId:scope==='document'?Boolean(active.documentId)&&e.documentId===active.documentId:Boolean(active.tabId)&&(e.tabId===active.tabId||e.documentId===active.documentId)));
   const sessions=new Map();for(const e of selected){const key=e.session;const row=sessions.get(key);if(row){row.count++;row.lastAt=e.timestamp;}else sessions.set(key,{session:key,tabId:e.tabId??null,documentId:e.documentId??null,mountId:e.mountId??null,componentId:e.componentId??null,build:e.build,count:1,firstAt:e.timestamp,lastAt:e.timestamp});}
   return clone({version:2,limits,scope,active,retired:data.retired,retainedTotal:data.entries.length,legacyWithoutIdentity:data.entries.filter(e=>!e.documentId).length,entries:selected,sessions:[...sessions.values()],warning});
  },
...
export function actionTraceCSV(trace){
 const rows=[['tabId','documentId','componentId','mountId','mountNumber','session','sequence','timestamp','build','action','source','accepted','reason','phaseBefore','phaseAfter','cardBefore','cardAfter','stationBefore','stationAfter','attempt','request','category','url','path','hash','visibility','wasDiscarded','navigationType','timeOrigin','detail'],...trace.entries.map(e=>[e.tabId,e.documentId,e.componentId,e.mountId,e.mountNumber,e.session,e.sequence,e.timestamp,e.build,e.action,e.source,e.accepted,e.reason,e.before.phase,e.after.phase,e.before.card,e.after.card,e.before.station,e.after.station,e.after.attempt??e.before.attempt,JSON.stringify(e.request??{}),e.category??'action',e.url,e.path,e.hash,e.visibility,e.wasDiscarded,e.navigationType,e.timeOrigin,JSON.stringify(e.detail??{})])];
