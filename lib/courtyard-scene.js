...
export function buildCourtyard(scene, root, materials, shadows, register, hole, options={}) {
  ...
  const box = (name, x, y, z, width, height, depth, material, solid = false) => {
    ...
    const featureKind=name.startsWith('mill-')?'mill':name.endsWith('-stack')?'lumber':name==='stack-body'?'lumber':name==='saw-carriage'?'carriage':name.startsWith('bell-')?'gantry':null;
    if(featureKind) mesh.metadata={...mesh.metadata,lineFeature:{id:`${name}:${x}:${z}`,kind:featureKind,label:featureKind==='mill'?(name==='mill-roof'?'MILL ROOF REBOUND':name==='mill-column'?'MILL POST REJECT':'MILL WALL REJECT'):featureKind==='lumber'?'LUMBER REBOUND':featureKind==='carriage'?'SAW CARRIAGE REJECT':'GANTRY REJECT'}};
