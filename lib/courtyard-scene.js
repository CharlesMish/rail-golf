import {SAW_BAY} from './line-lab.js';
...
export function buildCourtyard(scene, root, materials, shadows, register, hole, options={}) {
  ...
  const box = (name, x, y, z, width, height, depth, material, solid = false) => {
    ...
    if(featureKind) mesh.metadata={...mesh.metadata,lineFeature:{id:`${name}:${x}:${z}`,kind:featureKind,...(featureKind==='carriage'?{assembly:'saw'}:featureKind==='mill'?{assembly:'mill',assemblyLabel:'MILL REBOUND'}:featureKind==='lumber'?{assembly:`lumber:${x}:${z}`,assemblyLabel:'LUMBER REBOUND'}:{}),label:featureKind==='mill'?(name==='mill-roof'?'MILL ROOF REBOUND':name==='mill-column'?'MILL POST REJECT':'MILL WALL REJECT'):featureKind==='lumber'?'LUMBER REBOUND':featureKind==='carriage'?'SAW CARRIAGE REJECT':featureKind==='platform'?'LOADING PLATFORM REBOUND':'GANTRY REJECT'}};
  ...
  for (const step of CASCADE_STEPS) {
    ...
    tread.metadata = { cascadeStep: step.id,lineFeature:{id:step.id,kind:step.id,label:'TREAD '+number+' KICK',assembly:`lumber:${step.x}:${step.z}`,assemblyLabel:'LUMBER REBOUND'} };
  ...
  if(options.lineLab&&hole.station?.id==='saw'){
    // Low workshop planks mark an existing clear patch; no new collision/ground volume.
    for(let i=0;i<4;i++)box('saw-bay-plank',SAW_BAY.x,.06,SAW_BAY.z+3+i*1.1,10,.12,1,'timber');
    for(const dx of [-6,6])box('saw-bay-marker',SAW_BAY.x+dx,1.2,SAW_BAY.z+5,.25,2.4,.25,'machine');
  }
  let skyToken = null;
  {
