...
export function makeSurveyRecord(ticket,ledger){
 ...
 ledger.forEach((event,index)=>{
  const normalized=normalizeLineEvidence([event])[0];
...
export function createSurveyArchive(indexedDB,limits=SURVEY_LIMITS){
 let database;
 const open=()=>database??=new Promise((resolve,reject)=>{
  const request=indexedDB.open('rail-golf-line-survey',1);
