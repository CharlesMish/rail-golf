...
export type LineEvidence={kind:'contact'|'pad-activation'|'switch-use'|'ruling'|'token'|'redirect'|'rejected'|'termination'|'relationship';reason?:string;contactSeconds?:number;members?:string[];terminal?:boolean;feature?:string;label?:string;incoming?:VectorLike;outgoing?:VectorLike;turn?:number;freeSeconds?:number;separation?:number;surface?:string;body?:string;point?:VectorLike;targetHit?:boolean;state?:'A'|'B';count?:number};
...
export type RecordedLineReceipt={ruleSet:string;total:number;claimIds:string[];uniqueFeatureCount:number;secondary:number;ending:string};
