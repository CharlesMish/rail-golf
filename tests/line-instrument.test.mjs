...
const ledger=[contact('mill'),contact('bank-a'),contact('bank-b'),contact('step-a'),contact('step-b'),contact('step-c'),{kind:'pad-activation'},{kind:'switch-use',state:'B'},{kind:'ruling',targetHit:true}];
...
test('placeholder receipts are deterministic and independent of the source ledger',()=>{
 ...
 assert.deepEqual(a,b);assert.equal(a.total,1700);assert.equal(a.awards.length,5);assert.equal(JSON.stringify(ledger),before);
 assert.equal(a.nonCanonical,true);assert.equal(a.ignored[0].index,0);
 const changed=PLACEHOLDER_RULES.map(r=>({...r,points:1}));assert.equal(scoreLine(ledger,changed).total,5);
});
test('raw roof, floor and switch touches never score; chatter and repeated qualified families cannot farm',()=>{
 ...
 assert.equal(scoreLine([contact('floor-b'),contact('switch-b'),{kind:'token',surface:'sky'}]).total,0);
 assert.equal(scoreLine([...ledger,...ledger,...ledger]).total,1700);
...
test('lab saved evidence persists independently of production records and rejects missing lab environment',()=>{
 ...
 assert.equal(lab.environment.floor,'B');assert.equal(lab.environmentAfter.floor,'A');assert.equal(lab.build,share.build);assert.equal(scoreLine(lab.ledger).total,1700);
