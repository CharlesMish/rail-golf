...
export function LineReceipt({ledger,shot}:{ledger:LineEvidence[];shot?:SavedLine}){
 ...
 return <details className="line-receipt" open={ledger.length>0} key={ledger.length?`receipt-${ledger.length}`:'empty'}>
  ...
  <p>CLAIMS · {receipt.claimTotal}<br/>VARIETY · +{receipt.secondary} / 200 · {receipt.awards.filter(a=>a.tier!=='FINISH').length} non-finish claims · {receipt.uniqueFeatureCount} qualified features<br/>RUN · +{receipt.run} / 150 · {receipt.runDistance.toFixed(1)} m qualified free flight</p>
